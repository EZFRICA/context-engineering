...
class MemoryEngine:
    ...
    def add_memory(
        ...
        """
        Directly inserts a memory fact into OpaqueBank (manual entries are pre-approved).
    ...
    def mount_context(self, scope_id: str, query: str = None, limit: int = 5) -> str:
        """
        Retrieves relevant context for the given scope from OpaqueBank (approved facts only).
        ...
        try:
            ...
            # Filter by scope only (all facts in OpaqueBank are approved)
    ...
    def find_scopes(self, query: str = None, limit: int = 5) -> List[Dict[str, Any]]:
        """
        Finds relevant memory scopes (trips/topics) from OpaqueBank only.
