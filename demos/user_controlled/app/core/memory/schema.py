...
def init_inbox_schema():
    """
    Initializes the 'UserInbox' collection for proposed facts.
    """
    client = get_weaviate_client()
    try:
        if not client.collections.exists("UserInbox"):
            ...
            print("Created collection: UserInbox")
        else:
            print("Collection UserInbox already exists.")
    finally:
        client.close()

def init_bank_schema():
    """
    Initializes the 'UserBank' collection for approved facts.
    """
    client = get_weaviate_client()
    try:
        if not client.collections.exists("UserBank"):
            client.collections.create(
                name="UserBank",
                properties=[
                    Property(name="content", data_type=DataType.TEXT, tokenization=Tokenization.WORD),
            ...
            print("Created collection: UserBank")
        else:
            print("Collection UserBank already exists.")
    finally:
        client.close()
